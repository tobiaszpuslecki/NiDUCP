...
%# Kodowanie
    ...
    zeros_num = 4 - mod(signal_length_H,4);
        for i = 1:zeros_num
            signal_H(signal_length_H+i) = 0;
        end
    else

    zeros_num = 0;  %liczba dodatkowych zer
    end
    signal_length_H = signal_length_H + zeros_num;
    nwords = signal_length_H/4; %ilosc slow
    signal_H = vec2mat(signal_H, nwords); %zamiana na macierz po 4 slowa
...
%# Kanal
    ...
    encodedNoisedHamming = canalMakeNoise2D(encoded, signal_length_H, noiseProbability);
