...
%Wzi??em tak jak emilka, ?eby si? zgadzal format
...
signalx = reshape(signal,4,[])';                %przetworzenie sygnalu na postac (n x 4) wektorów - pociecie go

beforeEnc = gf(signalx)                         %argument sygnalu musi byc tablica Galois

encSignal = bchenc(beforeEnc,n,k)               %zakodowanie sygnalu w postaci (n x 7) wektorów
...
if afterDec(2,1) == 1
    afterDec(2,1) = 0;
else                            %Te 3 ify mo?esz wywali? s? podgl?dowo do zmiany warto?ci bitów w 3 s?owach
    afterDec(2,1) = 1;          %Jak uruchomisz z tymi ifami do ilo?? b??dów bedzie >= 3
end
